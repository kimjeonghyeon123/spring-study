...
<script type="text/javascript">
   ...
   if(msg == "CHAT_OUT_EXIST") {
	   alert(other_nickname + '님이 나간 채팅방이 존재합니다. 방을 나가면 새로운 채팅을 보낼 수 있습니다.')
   }
   if(msg == "CHAT_NOTEXIST") {
      alert(other_nickname + "님에게 채팅을 보낼 수 있습니다.")
   }
   ...
   $(document).ready(function(){
      ...
      $("#chatrooms").on('click', '.delBtn', function() {
         ...
         $.ajax({
            type: 'post',
            url: '/withdang/chat/deletechatting',
            headers: { "content-type" : "application/json" },  //요청헤더
   ...
   let toHtml = function(chatrooms) {
       ...
       return new Promise(function(resolve, reject){ 
          $.ajax({
             ...
             url: '/withdang/chat/showchattings',
               headers: { "content-type" : "application/json" },  //요청헤더
            data : JSON.stringify({chatroom_id:parseInt(chatroom_id), login_nickname: login_nickname}),
             success: function(result) {
                $('#chat').html(chatHtml(result.list))
                $('#chat-other_email').text(other_nickname + '님과의 대화')
                
                if(result.chatroomCnt !== 2) {
                   $('#chat').append('<div style="text-align: center;">' + other_nickname + '님이 방을 나갔습니다.</div>')
                   $('#msg-input').attr('disabled', true) // textarea 비활성화
                    $('#send-btn').attr('disabled', true)
                }
                else {
    ...
    let chatroomCnt = function(chatroom_id) {
       return new Promise(function(resolve, reject){ 
          $.ajax({
             type: 'post',
             url: '/withdang/chat/getchattingcnt',
               headers: { "content-type" : "application/json" },  //요청헤더
    ...
    async function removeChatting(chatroom_id) {
       try {
         ...
         $('#chat').html('')
          if(socket) {
            let socketMsg = "removechat," + other_nickname + "," + chatroom_id
