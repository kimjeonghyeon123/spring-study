...
public class JustTest {
	...
	public void inputTest() throws Exception {
        ...
        for (int i = 0; i < features.length(); i++) {
            ...
            sidoDao.insert(new SidoDTO(sido_code, sido_name));
        }
	}
	
	@Test
	public void inputTest2() throws Exception {
		List<SidoDTO> list = sidoDao.selectAll();
		
		for(SidoDTO sidoDTO : list) {
	        StringBuilder urlBuilder = new StringBuilder("https://api.vworld.kr/req/data"); /*URL*/
	        urlBuilder.append("?" + URLEncoder.encode("key","UTF-8") + "=430A9486-5035-3F51-B4BF-93CB0A1F4533"); /*Service Key*/
	        urlBuilder.append("&" + URLEncoder.encode("domain","UTF-8") + "=" + URLEncoder.encode("http://localhost:8080", "UTF-8"));
	        urlBuilder.append("&" + URLEncoder.encode("service","UTF-8") + "=" + URLEncoder.encode("data", "UTF-8"));
	        ...
	        String str = "sig_cd:like:"+sidoDTO.getSido_code();
	        ...
	        for (int i = 0; i < features.length(); i++) {
	            JSONObject feature = features.getJSONObject(i);
	            JSONObject properties = feature.getJSONObject("properties");
	            String sigoon_code = properties.getString("sig_cd");
	            String sigoon_name = properties.getString("sig_kor_nm");
	            
	            sigoonDao.insert(new SigoonDTO(sigoon_code, sigoon_name));
	        }
		}
	}
	...
	public void inputTest3() throws Exception {
		List<SigoonDTO> list = sigoonDao.selectAll();
		
		for(SigoonDTO sigoonDTO : list) {
	        StringBuilder urlBuilder = new StringBuilder("https://api.vworld.kr/req/data"); /*URL*/
	        urlBuilder.append("?" + URLEncoder.encode("key","UTF-8") + "=430A9486-5035-3F51-B4BF-93CB0A1F4533"); /*Service Key*/
	        urlBuilder.append("&" + URLEncoder.encode("domain","UTF-8") + "=" + URLEncoder.encode("http://localhost:8080", "UTF-8"));
	        urlBuilder.append("&" + URLEncoder.encode("service","UTF-8") + "=" + URLEncoder.encode("data", "UTF-8"));
	        ...
	        String str = "emd_cd:like:"+sigoonDTO.getSigoon_code();
	        urlBuilder.append("&" + URLEncoder.encode("attrfilter","UTF-8") + "=" + URLEncoder.encode(str, "UTF-8"));
	        ...
	        for (int i = 0; i < features.length(); i++) {
	            JSONObject feature = features.getJSONObject(i);
	            JSONObject properties = feature.getJSONObject("properties");
	            String dong_code = properties.getString("emd_cd");
	            String dong_name = properties.getString("emd_kor_nm");

	            dongDao.insert(new DongDTO(dong_code, dong_name));
	        }
		}
