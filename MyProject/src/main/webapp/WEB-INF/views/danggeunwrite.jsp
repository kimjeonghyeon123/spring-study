...
<body>
    ...
    <div class="board_wrap">
        ...
        <form class="board_write_wrap" id="form" action="" method='post' style="display: flex; flex-direction: column;">
            <div class="board_write">
                <div class="title">
                    <dl>
                        <dd><input type="text" name="title" value="${danggeunDTO.title}" placeholder="제목 입력"></dd>
                        <input type="hidden" name="id" value="${danggeunDTO.id}">
                    </dl>
                </div>
                <section class="dangguen-sec">
                    <div class="info">
                        <dl>
                            <dd><input type="number" name="price" value="${danggeunDTO.price}" placeholder="판매 가격"></dd>
                            <input type="text" name="writer_email" value="${loginEmail}" style="display: none;">
                        </dl>
                        <div id="dangguen-img">
			           	<select class="form-select" aria-label="category" name="type_id">
			                    <c:forEach var="DanggeunTypeDTO" items="${typeList}">
			                    	<option value="${DanggeunTypeDTO.id}"  ${danggeunDTO.type_id == DanggeunTypeDTO.id ? "selected" : ""}>${DanggeunTypeDTO.name}</option>
			                    </c:forEach>
			            </select>
                        ...
                        <input id="imageinput" name="image" type="file" multiple="multiple" accept=".jpg, .jpeg, .png" onchange="previewImage(this)">
                        <img id="preview" />
                    ...
                    <div class="cont">
                        <textarea name="content" placeholder="내용 입력" required>${danggeunDTO.content}</textarea>
                    </div>
                </section>
            </div>
            <div class="bt_wrap">
...
<script type="text/javascript">    
	let loginEmail = '${loginEmail}'
   	let sido_code = '${danggeunDTO.sido_code}'
    let sigoon_code = '${danggeunDTO.sigoon_code}'
    let dong_code = '${danggeunDTO.dong_code}'
	...
	let formCheck = function(frm) {
	    ...
	    if (frm.find('input[name="image"]').get(0).files.length === 0) {
	        alert('사진을 선택해주세요.');
	        return false;
	    }
	...
	$(document).ready(function(){        
        $("#writeBtn").on('click', function(){
        	let form = $('#form')
			form.attr("action", "/danggeun/danggeun/write")
        ...
        $("#modifyBtn").on('click', function(){
        	let form = $('#form')
			form.attr("action", "/danggeun/danggeun/modify")
			form.attr("method", "post")
			if(formCheck(form)) {
				form.submit()
			}       	
