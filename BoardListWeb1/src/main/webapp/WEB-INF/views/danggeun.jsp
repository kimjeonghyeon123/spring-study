	                        ...
	                        <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
	                            <div class="text-center"><a class="btn btn-outline-dark mt-auto" href="chat.html">구매하기</a>
									...
									<c:forEach var="storeId" items="${storeList}">
										<c:if test="${danggeundto.id == storeId}">
											<c:set var="count" value="${count + 1}"/>
										</c:if>
									</c:forEach>
									<c:if test="${count == 1}">
										<button type="button" class="btnscrap storeBtn" value="${danggeundto.id}" style="background-color: red;">찜♥</button>
									</c:if>
									<c:if test="${count != 1}">
										<button type="button" class="btnscrap storeBtn" value="${danggeundto.id}">찜♥</button>
									</c:if>
	                            </div>
	                        </div>
	                    </div>
	...
	$(document).ready(function(){
		$(".storeBtn").on("click", function(){
			...
			if(style == "background-color: red;") {
				...
				button.attr("style", "background-color: red;")
			}
