...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        if(chatroomId !== "") {
        	showChattingAndList(chatroomId)
        }
        else{ 
        	showListOnly()
        }
        ...
        $("#chatrooms").on('click', '.chatroom', function(){
        	...
        	showChattingAndListwithsocket(chatroomId)
        })
	    	...
	    	$.ajax({
	    		...
	    		success: function(result) {
	    			...
	    			if(result.chatroomCnt !== 2) {
		    			...
		    			$('#chatInput').attr('disabled', false) // textarea 활성화
    ...
    async function showChattingAndList(chatroomId) {
    	try {
    		await showChattingList(chatroomId)
    		await showList()
    	} catch(error){
    		alert('error')
    	}
    ...
    async function showChattingAndListwithsocket(chatroomId) {
    	try {
    		await chatroomCnt(chatroomId)
    		await showChattingList(chatroomId)
    		await showList()
    ...
    async function sendChatting(chatroomId) {
    	try {
    		await showChattingList(chatroomId)
    		await showList()
    ...
    async function removeChatting(chatroomId) {
    	try {
    		await showList()
